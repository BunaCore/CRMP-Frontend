        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="max-w-5xl mx-auto flex flex-col gap-5">
                ...
                return (
                  <div
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 py-3 min-w-[100px] cursor-pointer ${
                      active ? 'border-b-3 border-primary bg-primary/5' : 'border-b-3 border-transparent hover:bg-gray-50 dark:hover:bg-gray-800'
                    }`}
                  >
                    <span className={`flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold ${active ? 'bg-primary text-white' : 'bg-[#cfdbe7] text-[#4c739a]'}`}>
                      {step.number}
                    </span>
                    <p className={`text-xs font-semibold ${active ? 'text-primary' : 'text-[#4c739a]'}`}>{step.label}</p>
                  </div>
                )
              })}
            </div>
            ...
            <div className="bg-card-light dark:bg-card-dark rounded-lg shadow-sm border border-border-color dark:border-gray-800 overflow-hidden flex flex-col lg:flex-row">
              ...
              <div className="flex-1 p-5 flex flex-col gap-5 border-b lg:border-b-0 lg:border-r border-border-color dark:border-gray-800">
                ...
                <div className="grid gap-3">
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-text-main dark:text-gray-300">Search User</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div className="flex flex-col gap-1">
                      <label className="text-xs font-medium text-text-main dark:text-gray-300">Assign Role</label>
                      <div className="relative">
                    ...
                    <div className="flex items-end">
                      <button onClick={handleAddMember} className="w-full h-9 bg-[#137FEC] text-white text-sm font-medium rounded-md flex items-center justify-center gap-1.5">
                        <Plus size={16} /> Add
                      </button>
                    </div>
                  </div>
                ...
                <div className="mt-1 bg-blue-50 dark:bg-blue-900/20 rounded-md p-3 flex gap-2">
                  ...
                  <div className="text-xs">
                    ...
                    <p className="text-text-sub dark:text-blue-200 mt-0.5">
                      ...
                      <Link href="#" className="text-primary hover:underline font-medium">invite an external member</Link> via email.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 p-5 bg-slate-50/50 dark:bg-gray-900/30 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-text-main dark:text-white">Current Team <span className="text-text-sub font-normal ml-1 text-xs">({teamMembers.length})</span></h3>
                </div>
                <div className="flex flex-col gap-2">
                  {teamMembers.map(member => (
                    <div key={member.id} className={`flex items-start gap-3 p-3 rounded-md bg-white dark:bg-card-dark border ${member.isPrincipalInvestigator?'border-primary/30 relative overflow-hidden':'border-border-color dark:border-gray-700'}`}>
                      {member.isPrincipalInvestigator && <div className="absolute top-0 left-0 w-1 h-full bg-primary" />}
                      <div className="relative w-10 h-10 rounded-full overflow-hidden flex-shrink-0 bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-400 border border-gray-200 dark:border-gray-600">
                        {member.imageUrl ? <img src={member.imageUrl} alt={member.name} className="w-full h-full object-cover"/> : <User size={18}/>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-1.5 mb-0.5">
                          <h4 className="font-medium text-text-main dark:text-white text-sm truncate">{member.name}</h4>
                          <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium ${getRoleBadgeStyles(member.role)}`}>
                      ...
                      {member.isPrincipalInvestigator ? (
                        <div className="text-text-sub" title="Cannot remove PI"><Lock size={16} className="opacity-40 cursor-not-allowed" /></div>
                      ) : (
                        <button onClick={()=>handleRemoveMember(member.id)} className="text-text-sub hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 p-1.5 rounded transition-colors" title="Remove Member">
                          <Trash2 size={16}/>
                        </button>
                      )}
                    </div>
            ...
            <div className="p-4 bg-background-light dark:bg-card-dark border-t border-border-color dark:border-gray-800 flex flex-col-reverse sm:flex-row items-center justify-between gap-3">
              <Link href="/proposals/new/general-info" className="w-full sm:w-auto px-5 py-2 rounded-md border border-border-color dark:border-gray-600 text-text-main dark:text-white text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center justify-center gap-1.5">
                <ChevronLeft size={16} /> Back to General Info
              </Link>
              <button onClick={goToBudgetPage} className="w-full sm:w-auto px-6 py-2 rounded-md bg-[#137FEC] text-white text-sm font-medium shadow-sm hover:shadow flex items-center justify-center gap-1.5">
