...
export default function BudgetPage() {
  ...
  const handleDeleteRow = (id: string) => setItems(items.filter(item => item.id !== id));
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="max-w-5xl mx-auto flex flex-col gap-5">
              ...
              <div className="flex min-w-max">
                {steps.map((step) => (
                  ...
                  >
                    ...
                    <p className={`text-xs font-semibold tracking-[0.015em] ${step.active ? 'text-primary' : 'text-[#4c739a]'}`}>{step.label}</p>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start mb-6 gap-4 lg:gap-0">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1 leading-none">Financial Planning</h1>
                <p className="text-sm sm:text-base text-slate-500">Detail your resource requirements and justify institutional funding.</p>
              </div>
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-5 w-full sm:w-[260px]">
                ...
                <div className="flex items-baseline gap-1.5">
                  <span className="text-2xl sm:text-3xl font-extrabold text-slate-900 leading-none">${calculateProjectBudget()}</span>
              ...
              <table className="w-full text-[14px] text-left min-w-[600px]">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="px-4 sm:px-6 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Description</th>
                    <th className="px-4 sm:px-6 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Category</th>
                    <th className="px-4 sm:px-6 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Unit Cost ($)</th>
                    <th className="px-4 sm:px-6 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Qty</th>
                    <th className="px-4 sm:px-6 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Total ($)</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 group">
                      ...
                      <td className="px-4 sm:px-6 py-2 font-bold text-slate-900">{calculateTotal(item.unitCost, item.qty)}</td>
                      <td className="px-2 sm:px-4 py-2 text-center">
                        <button
                          onClick={() => handleDeleteRow(item.id)}
                          className="text-slate-300 hover:text-red-500 transition-colors"
                        >
                          <Trash2 className="w-4 sm:w-5 h-4 sm:h-5" strokeWidth={2} />
                        </button>
                      </td>
                    </tr>
                  ))}
