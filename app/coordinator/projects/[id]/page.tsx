...
export default function ProposalDetailPage() {
  ...
  const [decisionDialogOpen, setDecisionDialogOpen] = useState<
  ...
  const advisor =
    project && project.advisorId ? getUserById(project.advisorId) : null;
    ...
    <div className="space-y-6">
                ...
                project.status === "needs_revision" && (
                  <div className="p-4 bg-yellow-50/50 dark:bg-yellow-950/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                    <p className="text-sm text-yellow-700 dark:text-yellow-300">
                      Revision deadline:{" "}
                      {new Date(project.revisionDeadline).toLocaleDateString()}
                    </p>
                  </div>
                )}
            </CardContent>
      ...
      {decisionDialogOpen && (
        <Dialog
          ...
          onOpenChange={() => setDecisionDialogOpen(null)}
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
              ...
              <DialogDescription>
                {decisionDialogOpen === "approve" &&
                  "Add optional notes about the approval"}
                {decisionDialogOpen === "reject" &&
                  "Provide a reason for rejection"}
                {decisionDialogOpen === "revision" &&
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setDecisionDialogOpen(null)}
              >
                Cancel
              </Button>
              <Button
