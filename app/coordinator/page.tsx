...
import { Plus, AlertCircle, Users, FileText, CheckCircle2, Clock, Eye } from 'lucide-react';
import Link from 'next/link';
import { ProposalStatus, getSubmittedProposals, getNeedsRevisionProposals } from '@/lib/mockData';
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {stats.totalProposals > 0 ? ((stats.approved / stats.totalProposals) * 100).toFixed(0) : 0}% approval rate
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {getSubmittedProposals().length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">No submitted proposals</p>
          ) : (
            <div className="space-y-3">
              {getSubmittedProposals().slice(0, 5).map((project) => (
      ...
      {stats.needsRevision > 0 && (
        <Card className="border-yellow-200 dark:border-yellow-800">
          <CardHeader>
            ...
            <CardDescription>Students have been asked to revise their proposals</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {getNeedsRevisionProposals().map((project) => (
                <div key={project.id} className="flex items-center justify-between p-4 border border-yellow-200 dark:border-yellow-800 rounded-lg bg-yellow-50/50 dark:bg-yellow-950/20 hover:bg-yellow-100/50 dark:hover:bg-yellow-950/40 transition-colors">
                  <div className="flex-1">
                    <h3 className="font-medium text-foreground">{project.title}</h3>
